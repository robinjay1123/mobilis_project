...
class _ChatDetailScreenState extends State<ChatDetailScreen> {
  ...
  String? _resolvedVehicleImageUrl;
  ...
  void initState() {
    super.initState();
    _resolvedVehicleImageUrl = widget.recipientAvatar.trim().isNotEmpty
        ? widget.recipientAvatar.trim()
        : null;
    ...
    _bookingContextFuture.then((booking) {
      ...
      final img = booking?['vehicle_image_url']?.toString().trim();
      setState(() {
        if (status != null) _bookingStatus = status;
        if (img != null && img.isNotEmpty) _resolvedVehicleImageUrl = img;
      });
    });
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: widget.isCustomerService
            ...
            : Row(
                children: [
                  if (_resolvedVehicleImageUrl != null &&
                      _resolvedVehicleImageUrl!.isNotEmpty) ...[
                    Container(
                      width: 36,
                      height: 36,
                      ...
                      child: OptimizedNetworkImage(
                        imageUrl: _resolvedVehicleImageUrl!,
                        width: 36,
                        height: 36,
                        fit: BoxFit.cover,
                        borderRadius: BorderRadius.circular(10),
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            ] else if (_bookingStatus != null && _bookingStatus!.isNotEmpty) ...[
                              const SizedBox(width: 6),
                              Container(
                                padding: const EdgeInsets.symmetric(
                                  horizontal: 6,
  ...
  Widget _buildBookingActivityBanner(bool isDark) {
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshot) {
        ...
        final displayVehicleName = vehicleName.isNotEmpty
            ? vehicleName
            : (fallbackName.isNotEmpty ? fallbackName : 'Assigned Vehicle');
        final plateNumber = vehicle['plate_number']?.toString().trim() ?? '';

        final carImageUrl = booking['vehicle_image_url']?.toString().trim() ??
            _resolvedVehicleImageUrl ??
            '';
        ...
        final bool isOngoing = status == 'ongoing' ||
            ...
            status == 'in_progress' ||
            status == 'in_transit';
        final bool isApproved = status == 'approved' || status == 'confirmed';
        final bool isReadOnly = status == 'completed' ||
            status == 'returned' ||
            status == 'cancelled' ||
            status == 'canceled' ||
            status == 'rejected';

        final Color themeColor = isOngoing
            ? AppColors.success
            : (isApproved
                ? AppColors.primary
                : (isReadOnly ? Colors.purpleAccent : Colors.blueAccent));

        final String badgeTitle = isOngoing
            ? 'ONGOING TRIP'
            : (isApproved
                ? 'APPROVED BOOKING'
                : (status == 'driver_accepted'
                    ? 'DRIVER ASSIGNED'
                    : (isReadOnly ? 'PAST TRIP' : 'BOOKING DETAILS')));
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: themeColor.withValues(alpha: 0.38),
            ),
          ),
          child: Row(
            children: [
